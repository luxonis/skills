...
Read `PROJECT_BRIEF.md`, `POC_PLAN.md`, `DEVICE.md`, and source when present. None is
...
and one hard case. Take this from the brief or plan when they exist; otherwise ask only
what would change what you film.
...
from the project env. Do **not** record through the PoC or product app.
...
2. What to film (named from step 1).
3. Exact command, output directory `recordings/`.
4. How long / when to stop (`q` or `--seconds`).
5. The expected `.tar` path they should confirm.

Intended shape (match the script `--help` if it differs):
