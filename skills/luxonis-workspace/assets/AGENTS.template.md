...
- If offline, work from installed `--help` and local examples and name which facts are unverified.

## How this repo runs

- `oakctl` is the host toolchain (udev, inspect, host-run env, future host config).
- Prefer `oakctl run-script` for host runs when installed `--help` lists it as a local DepthAI environment runner; do not invent subcommands. If no host runner exists, run via this project's isolated env and still use oakctl for inspect and udev.
